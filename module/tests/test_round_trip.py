...
4. Compare every YAML in the two bundle trees (manifest, juan, annotation
   JSONL, edition manifest, edition juan) as Python dicts.
...
def round_trip_dirs(tmp_path_factory) -> tuple[Path, Path, Path, Path]:
    work = tmp_path_factory.mktemp("rt")

    # Bundle A from the original source.
    bundle_a = _read_input(REPO / "input" / "tls")
    a_root = work / "a"
    a_archive = work / "a-annotations"
    write_bundle(bundle_a, a_root, annotations_root=a_archive)
    ...
    b_root = work / "b"
    b_archive = work / "b-annotations"
    write_bundle(bundle_b, b_root, annotations_root=b_archive)
    b_bundle = b_root / TEXT_ID
    return a_bundle, b_bundle, a_archive, b_archive


def test_master_manifest_equal(round_trip_dirs: tuple[Path, Path, Path, Path]):
    a, b, _aa, _ba = round_trip_dirs
    assert _load(a / f"{TEXT_ID}.manifest.yaml") == _load(
        b / f"{TEXT_ID}.manifest.yaml"
    )


def test_master_juan_equal(round_trip_dirs: tuple[Path, Path, Path, Path]):
    a, b, _aa, _ba = round_trip_dirs
    assert _load(a / f"{TEXT_ID}_001.yaml") == _load(b / f"{TEXT_ID}_001.yaml")


def test_ann_file_equal(round_trip_dirs: tuple[Path, Path, Path, Path]):
    _a, _b, aa, ba = round_trip_dirs
    a_recs = _load_jsonl(aa / TEXT_ID / f"{TEXT_ID}_001.ann.jsonl")
    b_recs = _load_jsonl(ba / TEXT_ID / f"{TEXT_ID}_001.ann.jsonl")
    assert a_recs == b_recs


def test_edition_manifest_equal(round_trip_dirs: tuple[Path, Path, Path, Path]):
    a, b, _aa, _ba = round_trip_dirs
    rel = Path("editions") / "T" / f"{TEXT_ID}-T.manifest.yaml"
    assert _load(a / rel) == _load(b / rel)


def test_edition_juan_equal(round_trip_dirs: tuple[Path, Path, Path, Path]):
    a, b, _aa, _ba = round_trip_dirs
