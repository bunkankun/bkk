...
- **KRP** can be added on top of an existing TLS bundle. Its
  ...
  demoted to one edition among others under `editions/master/` — it
...
| **TLS-sourced bundle exists**        | **KRP**         | **Merge.** KRP editions written under `editions/`; the synthesized KRP master is demoted to `editions/master/`; the TLS root manifest's `editions:` list is extended. |
...
- The synthesized KRP master is written via `write_krp_edition` (not
  `write_krp_master`) so it lands under `editions/master/`. Variant +
...
- TLS → KRP via the CLI: TLS surface (master, sidecar, T edition) is
  byte-identical after the merge except for the master manifest's
  grown `editions:` list; KRP editions appear under `editions/`; the
  demoted master lands at `editions/master/`.
...
- **Recipe-pinned `master.branch` other than `"master"` is untested.**
  The KRP synthesis pipeline normally produces `edition_short="master"`
  for the master bundle regardless of the recipe's `master.branch`
  field, so the demoted edition always lands at `editions/master/`.
